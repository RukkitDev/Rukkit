package cn.rukkit.graphics;

/*
 * Copyright LWJGL. All rights reserved.
 * License terms: https://www.lwjgl.org/license
 */
// /usr/bin/env /usr/lib/jvm/java-21-openjdk-amd64/bin/java @/tmp/cp_6oprivefwpfjaxxczycohwun6.argfile -Djava.library.path=./data/native cn.rukkit.graphics.ColoredTriangleDemo
...
import static cn.rukkit.graphics.VKUtil.*;
...
public class ColoredTriangleDemo {
    ...
    private static Vertices createVertices(VkPhysicalDeviceMemoryProperties deviceMemoryProperties, VkDevice device) {
        ByteBuffer vertexBuffer = memAlloc(4 * (2 + 3) * 4);
    ...
    private static long createPipeline(VkDevice device, long renderPass, VkPipelineVertexInputStateCreateInfo vi) throws IOException {
        ...
        shaderStages.get(0).set(loadShader(device, "./data/vulkan/coloredTriangle.vert", VK_SHADER_STAGE_VERTEX_BIT));
        shaderStages.get(1).set(loadShader(device, "./data/vulkan/coloredTriangle.frag", VK_SHADER_STAGE_FRAGMENT_BIT));
    ...
    private static VkCommandBuffer[] createRenderCommandBuffers(VkDevice device, long commandPool, long[] framebuffers, long renderPass, int width, int height,
        ...
        for (int i = 0; i < renderCommandBuffers.length; ++i) {
            ...
            // Draw triangle
            vkCmdDraw(renderCommandBuffers[i], 4, 1, 0, 0);
